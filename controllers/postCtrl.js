...
const postCtrl = {
  ...
  getPosts: async (req, res) => {
    try {
      ...
      // const post = await features.query.sort("-createdAt").populate("user likes", "avatar username fullname followers").populate({path:"comments"}).populate({path:"user likes"});


      const post = await features.query
        .sort("-createdAt")
        .populate("user likes", "avatar username fullname followers").populate({path:"comments",populate:{path:"user"}})
        // populate({
        //   path: "comments",
        //   populate: {
        //     path: "user likes ",
  ...
  getUserPosts: async (req, res) => {
    console.log("im in get user posts");
    try {
      const features = new APIfeatures(
  ...
  getPost: async (req, res) => {
    ...
    // .populate("user likes", "avatar username fullname followers").populate({path:"comments"}).populate({ path: "user likes"});
            

    // const post = await features.query
    // .sort("-createdAt")
    // .populate("user likes", "avatar username fullname followers").populate({path:"comments",populate:{path:"user"}})
    const post = await Posts.findById(req.params.id).populate("user likes", "avatar username fullname followers").populate({path:"comments",populate:{path:"user likes"}})
